...
import { Sparkles, Layers, Mic, Sliders, ShieldCheck, Activity } from 'lucide-react';
...
const WORKSPACES: { id: WorkspaceTab; label: string; icon: React.ReactNode; color: string; desc: string }[] = [
  {
    ...
    color: 'amber',
    desc: 'Capture, Beatbox & 64-Step Grid',
  },
  {
    id: 'BUILD',
    label: '2. BUILD',
    ...
    color: 'cyan',
    desc: 'Arrangement, Timeline & Realization',
  },
  {
    id: 'WRITE_RECORD',
...
export const WorkspaceNav: React.FC<WorkspaceNavProps> = ({ activeWorkspace, onSelectWorkspace }) => {
  return (
    <div className="w-full bg-slate-900/90 backdrop-blur border-b border-slate-800 px-4 py-2 flex flex-wrap items-center justify-between gap-2 shadow-lg">
